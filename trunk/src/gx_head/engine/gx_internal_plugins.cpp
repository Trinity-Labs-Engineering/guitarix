...
bool smbPitchShift::setParameters(int sampleRate_)
{
    ...
    if (mem_allocated) {
        ready = false;
        try {
            pitch_shifter.prepare(sampleRate, latency);
        } catch(...) {
            gx_print_error("pitch shift", "cannot change sample rate");
            return false;
        }
        ready = true;
    }
...
smbPitchShift::smbPitchShift(EngineControl& engine_, sigc::slot<void> sync_):
  ...
  ready(false),
  semitones(0.0f),
  a(1.0f), b(1.0f), c(1.0f), d(1.0f), l(0.0f),
  wet(100.0f),
  dry(0.0f),
  octave(0),
  sampleRate(48000),
  ...
  plugin() {
    ...
    name = N_("Pitch Shift");
    groups = 0;
    description = N_("low latency polyphonic pitch shift");
    category = N_("Misc");       // category
    mono_audio = compute_static;
    stereo_audio = 0;
    set_samplerate = init;
...
void smbPitchShift::clear_state_static(PluginDef *p)
{
    ...
    // this hook only after that activation and can therefore clear delay and
    // crossfade history without reallocating the prepared ring buffers.
...
void smbPitchShift::mem_alloc()
{
    ...
    try {
        ...
        gx_print_error("pitch shift", "cannot allocate memory pool");
        return;
    }
    mem_allocated = true;
    ready = true;
...
{
    // The time-domain engine is block-size independent, so a JACK period
    // change does not require any reconfiguration or allocation.
    (void)size;
}
...
void smbPitchShift::change_latency()
{
    ...
    if (mem_allocated) {
        try {
            ...
            gx_print_error("pitch shift", "cannot change quality setting");
        }
    }
}
...
void always_inline smbPitchShift::PitchShift(int count, float *indata, float *outdata)
{
    ...
    int octave_semitones = 0;
    if (octave == 1) octave_semitones = 12;
    else if (octave == 2) octave_semitones = -12;
    const double ratio = pow(2.0,
        (semitones + octave_semitones)*(1.0/12.0));
    pitch_shifter.process(indata, outdata, count, ratio,
        0.01f*wet, 0.01f*dry, l >= 0.5f, a, b, c, d);
}
...
int smbPitchShift::register_par(const ParamReg& reg) 
{
    ...
    reg.registerFloatVar("smbPitchShift.wet", N_("Wet"), "S", N_("Wet amount"), &wet, 100.0, 0.0, 100.0, 1, 0);
    reg.registerFloatVar("smbPitchShift.dry", N_("Dry"), "S", N_("Dry amount"), &dry, 0.0, 0.0, 100.0, 1, 0);
